//This was provided, I use it for the matrix manipulation, I have also added in a few at origin ortations, but did not end up using them.
...
public final class Mat4Transform  {   // row column formulation
    ...
    public static Mat4 rotateAroundYAtOrigin(float angle, Mat4 current) {
        ...
        current.set(1, 3, 0);
        current.set(2, 3, 0);
        current = Mat4.multiply(current, Mat4Transform.rotateAroundY(angle));
    ...
    public static Mat4 rotateAroundZAtOrigin(float angle, Mat4 current) {
        float xpos = current.get(0, 3);
        float ypos = current.get(1, 3);
        float zpos = current.get(2, 3);
        current.set(0, 3, 0);
        current.set(1, 3, 0);
        current.set(2, 3, 0);
        current = Mat4.multiply(current, Mat4Transform.rotateAroundZ(angle));
        current.set(0, 3, xpos);
        current.set(1, 3, ypos);
        current.set(2, 3, zpos);
        return current;
